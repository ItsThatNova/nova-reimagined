...
void main() {
    ...
    float lViewPos1 = length(viewPos1.xyz);
        ...
        bool doVolumetricLight = true;

        #if LIGHTSHAFT_LOCATION == 0
            doVolumetricLight = (isEyeInWater != 1);
        #elif LIGHTSHAFT_LOCATION == 1
            doVolumetricLight = (isEyeInWater == 1);
        #endif

        if (doVolumetricLight) {
    ...
    if (isEyeInWater == 1) {
        ...
        float underwaterLightshaftBrightness =
            uwbfDayWeight   * UNDERWATERLIGHTSHAFT_DAY_BRIGHTNESS   * 0.01 +
            uwbfDuskWeight  * UNDERWATERLIGHTSHAFT_DUSK_BRIGHTNESS  * 0.01 +
            uwbfNightWeight * UNDERWATERLIGHTSHAFT_NIGHT_BRIGHTNESS * 0.01;

        if (z0 == 1.0) color.rgb = waterFogColor * underwaterBorderFogBrightness;

        vec3 underwaterMult = vec3(0.80, 0.87, 0.97);
        color.rgb *= underwaterMult * 0.85;
        volumetricEffect.rgb *= pow2(underwaterMult * 0.71) * underwaterLightshaftBrightness;
