...
#ifdef ANY_SURFACE_SNOW
    #include "/lib/materials/materialMethods/leafSnow.glsl"
#endif
...
void main() {
    ...
    #ifdef ANY_SURFACE_SNOW
        DoSurfaceSnow(color, smoothnessG, highlightMult, smoothnessD, emission,
                      playerPos, lmCoord, snowMinNdotU, NdotU, subsurfaceMode, mat);
    #endif
    ...
    float skyLightFactor = GetSkyLightFactor(lmCoordM, shadowMult);
        ...
        // Vanilla fades out at the render edge so DH can dissolve in underneath
        float vanillaDist = length(playerPos.xz);
        float vanillaFade = smoothstep(far * 0.98, far * 1.03, vanillaDist);
        if (vanillaFade > dither) discard;
    #endif
