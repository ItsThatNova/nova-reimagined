...
void main() {
    ...
    if (z0 < 1.0) {
        ...
        #ifdef DARK_OUTLINE
            #ifdef DARK_OUTLINE_FADE
                float outlineFade = 1.0 - smoothstep(float(DARK_OUTLINE_FADE_START), float(DARK_OUTLINE_FADE_END), lViewPos);
            #else
                float outlineFade = 1.0;
            #endif
            #if defined DISTANT_HORIZONS && defined DH_OVERDRAW_PROTECTION
                // Suppress outline in the DH dither transition zone to avoid
                // dark line where discarded vanilla fragments leave holes in depthtex0
                outlineFade *= 1.0 - smoothstep(far * 0.90, far * 0.98, lViewPos);
            #endif
            if (outlineFade > 0.001) {
                #ifndef DARK_OUTLINE_ON_PARTICLES
                    if (!entityOrParticle)
                #endif
                DoDarkOutline(color.rgb, z0, outlineFade);
            }
        #endif
        ...
        DoFog(color, skyFade, lViewPos, playerPos, VdotU, VdotS, dither, false, 0.0);
            ...
            if (z0DH < 1.0) { // Distant Horizons Chunks
                ...
                #ifdef DARK_OUTLINE
                    // Fade outline from full strength at DARK_OUTLINE_FADE_START to zero at DARK_OUTLINE_FADE_END
                    #ifdef DARK_OUTLINE_FADE
                        float outlineFade = 1.0 - smoothstep(float(DARK_OUTLINE_FADE_START), float(DARK_OUTLINE_FADE_END), lViewPos);
                    #else
                        float outlineFade = 1.0;
                    #endif
                    // Fade outline during rain/snow to match atmospheric visibility
                    outlineFade *= 1.0 - rainFactor2;
                    if (outlineFade > 0.001) {
                        DoDarkOutlineDH(color.rgb, z0DH, outlineFade);
                    }
                #endif
    ...
    waterRefColor = sqrt(waterRefColor) * 0.5;
        ...
        if (isEyeInWater == 0) {
            ...
            float rainTimedAFD = mix(RAIN_NIGHT_ATM_FOG,
                                     mix(RAIN_DUSK_ATM_FOG, RAIN_DAY_ATM_FOG, noonFactor),
                                     sunVisibility2);
            float biomeFogD = max(inSnowy, inDry) * BIOME_WEATHER_FOG_I;
            float factor = clamp(rainTimedAFD + biomeFogD, 0.0, 2.0);
