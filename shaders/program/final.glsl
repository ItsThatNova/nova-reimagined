...
//Common Functions//
    ...
    #if SHARPENING_MODE == 1
        // AMD FidelityFX Contrast Adaptive Sharpening (CAS)
        // Core algorithm by Timothy Lottes, adapted for GLSL without upscaling.
        // Adaptive: high-contrast areas are sharpened less to avoid ringing/haloing.
        void SharpenImage(inout vec3 color, vec2 uv) {
            // Sample center and 4 cardinal neighbours
            vec3 a = texture2D(colortex3, uv + vec2( 0.0,     -viewD.y)).rgb; // N
            vec3 b = texture2D(colortex3, uv + vec2(-viewD.x,  0.0    )).rgb; // W
            vec3 c = color;                                                     // center
            vec3 d = texture2D(colortex3, uv + vec2( viewD.x,  0.0    )).rgb; // E
            vec3 e = texture2D(colortex3, uv + vec2( 0.0,      viewD.y)).rgb; // S

            // Min/max of the 5 samples per channel
            vec3 mnRGB = min(min(min(a, b), c), min(d, e));
            vec3 mxRGB = max(max(max(a, b), c), max(d, e));

            // Adaptive weight: reduces sharpening where contrast is already high.
            // clamp prevents division by zero on pure black.
            vec3 rcpMxRGB = 1.0 / max(mxRGB, 0.001);
            vec3 ampRGB = clamp(min(mnRGB, 2.0 - mxRGB) * rcpMxRGB, 0.0, 1.0);

            // IMAGE_SHARPENING 0-20 maps to 0.0-1.0 CAS strength (peak 0.0 to -0.125).
            float peak = -0.125 * (IMAGE_SHARPENING / 20.0);
            vec3 w = sqrt(ampRGB) * peak;

            // Apply filter and clamp to valid range
            color = max((c + w * (a + b + d + e)) / (1.0 + 4.0 * w), vec3(0.0));
        }
    #else
        // Original Complementary unsharp mask sharpener
