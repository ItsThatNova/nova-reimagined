...
#ifdef OTHER_TONEMAP
    #include "/lib/misc/tonemapOperators.glsl"
#endif
...
void main() {
    ...
    #ifdef OTHER_TONEMAP
        // TM_EXPOSURE intentionally not applied here.
        // Each operator handles its own exposure pre-scale internally.
        color = ApplyTonemapOperator(color);
        LinearToRGB(color);
        color = clamp01(color);
    #else
        DoCompTonemap(color); // TM_EXPOSURE applied inside DoCompTonemap
    ...
    #ifndef OTHER_TONEMAP
        // BSL saturation is tuned for the Complementary curve — skip for other operators
