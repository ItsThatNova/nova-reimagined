...
void main() {
    vec4 color = vec4(glColor.rgb, 1.0);
        ...
        color.rgb *= mix(1.0, WEATHER_SKY_BRIGHTNESS, rainFactor);
        ...
        color.rgb += GetStars(starCoord, VdotU, VdotS);
            ...
            #endif
            // Raise the threshold near the horizon so the disc appears at most
            // ~15% smaller (tighter) there than at noon. smoothstep starts at
            // SdotU=0.45 (sun ~27° above horizon) and is fully applied at SdotU=0.05.
            // abs() makes this apply symmetrically to both sun and moon.
            float horizonProximity = smoothstep(0.45, 0.05, abs(SdotU));
            float dynamicSunSizeFactor1 = sunSizeFactor1
                + horizonProximity * (1.0 - sunSizeFactor1) * 0.10;

            if (absVdotS > dynamicSunSizeFactor1) {
                float sunMoonMixer = sqrt1(sunSizeFactor2 * (absVdotS - dynamicSunSizeFactor1));
                ...
                if (VdotS > 0.0) {
                    ...
                    sunMoonMixer = clamp(sunMoonMixer, 0.0, 1.0); // guard against single-frame spikes
