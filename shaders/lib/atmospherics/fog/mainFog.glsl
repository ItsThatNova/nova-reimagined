    ...
    void DoBorderFog(inout vec4 color, inout float skyFade, float lPos, float VdotU, float VdotS, float dither) {
        ...
        if (fog > 0.0) {
            fog = clamp(fog, 0.0, 1.0) * BORDER_FOG_I;

            #ifdef OVERWORLD
                vec3 fogColorM = GetSky(VdotU, VdotS, dither, true, false);
                float thunderFactor = GetThunderFactor();
                float bfNightWeight = 1.0 - sunVisibility2;
                float bfDuskWeight  = sunVisibility2 * (1.0 - noonFactor);
                float bfDayWeight   = sunVisibility2 * noonFactor;
                float borderFogBrightness = mix(
                    bfDayWeight   * RAIN_DAY_BORDER_FOG_BRIGHTNESS  +
                    bfDuskWeight  * RAIN_DUSK_BORDER_FOG_BRIGHTNESS +
                    bfNightWeight * RAIN_NIGHT_BORDER_FOG_BRIGHTNESS,
                    bfDayWeight   * STORM_DAY_BORDER_FOG_BRIGHTNESS  +
                    bfDuskWeight  * STORM_DUSK_BORDER_FOG_BRIGHTNESS +
                    bfNightWeight * STORM_NIGHT_BORDER_FOG_BRIGHTNESS,
                    thunderFactor);
                fogColorM *= mix(1.0, borderFogBrightness, rainFactor);
                ...
                if (isEyeInWater == 1) {
                    ...
                    fogColorM *= underwaterBorderFogBrightness;
                }
            #elif defined NETHER
    ...
    void DoAtmosphericFog(inout vec4 color, vec3 playerPos, float lViewPos, float VdotS) {
            ...
            // Base fog — RAIN_BASE_FOG applies equally to rain and storm.
            float thunderFactor = GetThunderFactor();
            float rbf = rainFactor * RAIN_BASE_FOG;
            float rbf2 = rbf * rbf;
            float fog = 1.0 - exp(-pow(lViewPos * (0.001 - 0.0007 * rbf), 2.0 - rbf2) * lViewPos * renDisFactor);
        #else
            float thunderFactor = GetThunderFactor();
            float rbf = rainFactor * RAIN_BASE_FOG;
            float fog = pow2(1.0 - exp(-max0(lViewPos - 40.0) * (0.7 + 0.7 * rbf) / ATM_FOG_DISTANCE));
        #endif

        // Base fog multiplier: scales from clear baseline up to rain maximum.
        float invRbf = 1.0 - rbf;
        fog *= ATM_FOG_MULT - 0.1 - 0.15 * invRbf;
        ...
        float altitudeFactorRaw = GetAtmFogAltitudeFactor(playerPos.y + cameraPosition.y);
            ...
            // Atmospheric fog density during rain — driven by RAIN_X_ATM_FOG sliders.
            // Default 0.0 matches original Style 1 (no extra fog).
            // Biome-specific fog (snowy/dry) adds on top, scaled by BIOME_WEATHER_FOG_I.
            // At 0.0 biome weather adds no extra fog. At 1.0 full original effect.
            float rainTimedAF  = mix(RAIN_NIGHT_ATM_FOG,  mix(RAIN_DUSK_ATM_FOG,  RAIN_DAY_ATM_FOG,  noonFactor), sunVisibility2);
            float stormTimedAF = mix(STORM_NIGHT_ATM_FOG, mix(STORM_DUSK_ATM_FOG, STORM_DAY_ATM_FOG, noonFactor), sunVisibility2);
            rainTimedAF = mix(rainTimedAF, stormTimedAF, thunderFactor);
            float biomeFog     = max(inSnowy, inDry) * BIOME_WEATHER_FOG_I;
            float atmFogFactor = clamp(rainTimedAF + biomeFog, 0.0, 2.0);
            if (isEyeInWater == 0 && atmFogFactor > 0.001) {
                float fogFactor = 4.0 + 2.0 * inDry * BIOME_WEATHER_FOG_I;
                fogFactor *= 0.5 + 0.5 * sunVisibility;

                float fogIntense = pow2(1.0 - exp(-lViewPos * fogFactor / ATM_FOG_DISTANCE));
                fog = mix(fog, fogIntense / altitudeFactor, 0.8 * rainFactor * atmFogFactor);
            }
...
void DoWaterFog(inout vec4 color, float lViewPos) {
    ...
    float uwfNightWeight = 1.0 - sunVisibility2;
    float uwfDuskWeight  = sunVisibility2 * (1.0 - noonFactor);
    float uwfDayWeight   = sunVisibility2 * noonFactor;
    float uwFogBrightMult = uwfDayWeight   * UNDERWATERFOG_DAY_BRIGHTNESS   * 0.01
                          + uwfDuskWeight  * UNDERWATERFOG_DUSK_BRIGHTNESS  * 0.01
                          + uwfNightWeight * UNDERWATERFOG_NIGHT_BRIGHTNESS * 0.01;

    color = mix(color, vec4(waterFogColor * uwFogBrightMult, 0), fog);
}
