...
vec4 GetClouds(inout float cloudLinearDepth, float skyFade, vec3 cameraPosOffset, vec3 playerPos,
    ...
    if (clouds.a > 0.0 && rainFactor > 0.0) {
        ...
        float cloudBrightness = mix(
            bfDayWeight   * RAIN_DAY_CLOUD_BRIGHTNESS  +
            bfDuskWeight  * RAIN_DUSK_CLOUD_BRIGHTNESS +
            bfNightWeight * RAIN_NIGHT_CLOUD_BRIGHTNESS,
            bfDayWeight   * STORM_DAY_CLOUD_BRIGHTNESS  +
            bfDuskWeight  * STORM_DUSK_CLOUD_BRIGHTNESS +
            bfNightWeight * STORM_NIGHT_CLOUD_BRIGHTNESS,
            thunderFactor);
        clouds.rgb *= mix(1.0, cloudBrightness, rainFactor);
    }
