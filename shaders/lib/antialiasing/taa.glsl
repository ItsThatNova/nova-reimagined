...
// YCoCg conversion functions
// Cheap to compute — just additions/subtractions, no matrix multiply
...
void NeighbourhoodClamping(vec3 color, inout vec3 tempColor, float z0, float z1, inout float edge) {
    // Convert current pixel to YCoCg for tighter, more accurate AABB clamping
    vec3 colorYCoCg = RGBToYCoCg(color);
    vec3 minclr = colorYCoCg;
    vec3 maxclr = colorYCoCg;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Sample neighbour in YCoCg space for tighter bounds
        vec3 clr = RGBToYCoCg(texelFetch(colortex3, texelCoordM2, 0).rgb);
        minclr = min(minclr, clr);
        maxclr = max(maxclr, clr);
    }

    // Clip history in YCoCg space then convert back to RGB
