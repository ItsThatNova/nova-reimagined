// ============================== Step 1: Color Prep ============================== //
    ...
    #if WATER_STYLE < 3 && !defined DH_WATER
        // Above 1.0: reduce exponent from pow2 toward linear for more visible pattern
        // Below 1.0: blend from pow2 toward flat neutral to reduce pattern without darkening
        vec3 colorPMFull = pow(colorP.rgb, vec3(2.0 / max(WATER_TEX_DETAIL_I, 1.0)));
        float colorPMFullLuma = max(dot(colorPMFull, vec3(0.299, 0.587, 0.114)), 0.0001);
        vec3 colorPM = mix(vec3(colorPMFullLuma), colorPMFull, min(WATER_TEX_DETAIL_I, 1.0));
        float colorPMLuma = max(dot(colorPM, vec3(0.299, 0.587, 0.114)), 0.0001);
        vec3 colorPMGray = mix(vec3(colorPMLuma), colorPM, WATER_TEX_COLOR_I);
        color.rgb = colorPMGray * glColorM * mix(1.0, colorPMLuma * 4.0, WATER_TEX_LUMA_I);
    #elif !defined DH_WATER
        vec3 colorPM = vec3(0.25);
        color.rgb = 0.375 * glColorM;
    #else
        // DH water: use flat grey path matching original CR DH behaviour (pow2(0.85) = 0.7225)
        // Approximate average vanilla texture luma so WATER_TEX_LUMA_I tracks vanilla water
        vec3 colorPM = pow2(colorP.rgb);
        float dhTexLuma = 0.52;
        color.rgb = colorPM * glColorM * mix(1.0, dhTexLuma * 4.0, WATER_TEX_LUMA_I);
    #endif
#else
    #if WATER_STYLE < 3
