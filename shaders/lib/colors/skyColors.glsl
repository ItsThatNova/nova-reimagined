    ...
    #define INCLUDE_SKY_COLORS
        ...
        // Biome-specific sky colour shifts — always active, scaled by BIOME_WEATHER_COLOR_I.
        vec3 nmscSnowM = inSnowy * vec3(-0.1, 0.3, 0.6)   * BIOME_WEATHER_COLOR_I;
        vec3 nmscDryM  = inDry   * vec3(-0.1, -0.2, -0.3) * BIOME_WEATHER_COLOR_I;
        vec3 ndscSnowM = inSnowy * vec3(-0.25, -0.01, 0.25) * BIOME_WEATHER_COLOR_I;
        vec3 ndscDryM  = inDry   * vec3(-0.05, -0.09, -0.1) * BIOME_WEATHER_COLOR_I;
        // Sky colour shift during rain removed — use Sky Color Multipliers in Atmosphere settings.
